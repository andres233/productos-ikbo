...
export class AuthService {
  ...
  public async register({ email, password, lastname, firstname }: RegisterRequestDto): Promise<RegisterResponse> {
  ...
  public async login({ email, password }: LoginRequestDto): Promise<LoginResponse> {
    ...
    if (!auth) {
      return { status: HttpStatus.NOT_FOUND, error: ['E-Mail not found'], token: null , user: null};
    }

    const isPasswordValid: boolean = this.jwtService.isPasswordValid(password, auth.password);

    if (!isPasswordValid) {
      return { status: HttpStatus.NOT_FOUND, error: ['Password wrong'], token: null, user: null };
    }

    const token: string = this.jwtService.generateToken(auth);

    return { token, status: HttpStatus.OK, error: null, user: auth};
  }
